...
**Version 1.1 (2026-09-13).** A self-contained technical specification of the equation, the numerical scheme used to integrate it, the implementation, the validation protocol, the observables, and the parameter regimes. Reproducible end-to-end from this document alone.

Version 1.1 aligns the document with the rule of the complete equation (§1.6), stated by the author on 2026-09-13. Version 1.0 described ablation modes as controls and made three of its validation criteria depend on them; those passages are withdrawn here, not silently rewritten. The full v1.0 text is preserved at `/Users/leo/workspace/archive/triad-equation-reference-v1.0-2026-09-13/`. Every place where v1.1 departs from v1.0 is marked **[v1.1]** in the text.
...
There is no claim made in this document beyond what is stated. The equation is defined, its terms are sourced, the algorithm is specified, and the diagnostics are enumerated. What the outputs mean for any specific scientific question is outside the scope of this reference.
...
| $\Gamma$ | $> 0$ **[v1.1]** | dissipation rate; the P3 bath is always on |
...
| $f_{\text{FDT}} = 2\gamma_0 k_B T$ | $> 0$ **[v1.1]** | FDT noise amplitude; $\eta$ is always active. Under the FDT lock its effective value is derived from $\Gamma$ and $k_B T$ (§1.3) |
| $k_B T$ | $> 0$ **[v1.1]** | bath temperature; the temperature dial under the FDT lock (§1.3) |
| $M$ | integer $\geq 3$ **[v1.1]** | number of memory modes; the P2 memory field carries at least three time-scales |

**[v1.1]** v1.0 admitted $\Gamma = 0$, $f_{\text{FDT}} = 0$ and any positive $M$. The reference implementation refuses them (§1.6).
...
The composite quantity $f_{\text{FDT}} = 2\gamma_0 k_B T$ is the FDT noise amplitude. The "lock" is the requirement that this amplitude is not independently tunable from $\Gamma$ when both are derived from the same bath.

**[v1.1] The lock as implemented.** The reference implementation (`fdt_couple = True`, the default) identifies the bath friction with the dissipation of equation (1), $\gamma_0 = \Gamma / \hbar$, and derives the effective amplitude on the grid as

$$f_{\text{FDT}}^{\,\text{eff}} = \frac{2 \, \Gamma \, k_B T}{\hbar} \, dx^D. \tag{3c}$$

Under the lock, $k_B T$ is the temperature dial and $\Gamma$ the friction dial. The magnitude of $\eta$ comes from (3c); the declared $f_{\text{FDT}}$ must be positive because $\eta$ is a term of (1) and is never off. This is the convention of the equation. The implementation flag `fdt_couple = False`, which bypasses (3c), is not part of the reference and must not be used to report results. v1.0 described $f_{\text{FDT}}$ as the exposed dial; that description is withdrawn.

### 1.4 Discretisation of the noise

On the grid, the per-step stochastic increment to $\Psi$ at grid point $i$ is

$$\Psi_i \leftarrow \Psi_i + \sqrt{\frac{f_{\text{FDT}}^{\,\text{eff}} \, dt}{dx^D}} \cdot \frac{\xi_i + i \, \xi'_i}{\sqrt{2}}, \tag{4}$$

with $\xi_i, \xi'_i \sim \mathcal{N}(0, 1)$ independent. The factor $dx^{-D}$ is the grid form of $\delta^{(D)}(\mathbf{x} - \mathbf{x}')$, and it is applied identically in every dimension and code path of the reference implementation.

**[v1.1] Consequence of the lock.** Substituting (3c) into (4), the $dx^D$ factors cancel and the per-site increment under the default lock is

$$\Psi_i \leftarrow \Psi_i + \sqrt{\frac{2 \, \Gamma \, k_B T \, dt}{\hbar}} \cdot \frac{\xi_i + i \, \xi'_i}{\sqrt{2}}, \tag{4'}$$

independent of the grid spacing. This is the convention of the reference implementation, applied identically in every dimension and code path. v1.0 called the absence of a $dx^{-D/2}$ scaling on the per-site amplitude "a frequent implementation error"; that sentence is withdrawn.
...
### 1.6 Rule of the complete equation **[v1.1]**

Stated by the author on 2026-09-13, verbatim: *"regra sou eu quem dito. é pra recusar sim. não isolamento, não falsificação. apenas a equação completa."*

Operationally:

1. Equation (1) is integrated only in full. There is no mode that disables a term. There are no ablation controls.
2. An implementation **refuses** a parameter set that switches a term off. In the reference implementation the refusals are: any `mode` other than the single complete one; $\Gamma \leq 0$; $f_{\text{FDT}} \leq 0$; $k_B T \leq 0$; fewer than three memory time-scales ($M < 3$); `len(nu) != len(lam)`. The refusal is a `ValueError` at parameter construction and again at the entry of every integrator; it cannot be bypassed through the public interface.

   The three bath refusals protect one term, $\eta$: $f_{\text{FDT}} = 0$ makes $\eta \equiv 0$ and deletes it from (1), exactly as $\Gamma = 0$ deletes $-i\Gamma$. $\Gamma$, $f_{\text{FDT}}$ and $k_B T$ are each a path by which the bath could be silenced; each is closed. What is protected is the term, not the number.
3. Nothing in the output is fabricated, tuned to a target after the fact, or replaced by a literal.
4. Raw results, declared parameters and seeds are preserved as produced.

The reference implementation is the statement of what the rule refuses. Its refusals are the ones listed in item 2 and no others.

Everything in this document that presupposed an ablation mode (v1.0 §6, §7.1–7.3, §7.6, Appendix A.2–A.3, Appendix C, Appendix D) is marked **[v1.1]** where it was withdrawn or reframed.
...
$\sigma = 2$ recovers the standard Laplacian $-\Delta$; the fractional operator adds an additional dispersive channel. **[v1.1]** The v1.0 instruction "set $\alpha = 0$ to disable" is withdrawn (§1.6).
...
## 6. Ablation modes — withdrawn **[v1.1]**

v1.0 of this section exposed three solver modes (`linear`, `thermal`, `full`) and six individual-term ablations (`no_memory`, `no_cubic`, `no_fractional`, `no_noise`, `no_dissipation`, `no_vext`) as controls. Under the rule of the complete equation (§1.6) there are no controls of this kind: the equation is integrated whole or not at all, and an implementation refuses a parameter set that switches a term off.

Concretely, the reference implementation accepts a single mode and rejects the v1.0 names:

| v1.0 mode / ablation | Status in v1.1 |
|---|---|
| `mode = "linear"` | refused (`mode` must be the single complete mode) |
| `mode = "thermal"` | refused |
| `mode = "full"` | refused as a name; its content — all terms active — is the only mode and needs no name |
| `no_noise` ($f_{\text{FDT}} = 0$) | refused |
| `no_dissipation` ($\Gamma = 0$) | refused |
| `no_memory`, `no_cubic`, `no_fractional`, `no_vext` | withdrawn as named controls; there are no controls |

The v1.0 text of this section is preserved in the archived copy named in the header.

---

## 7. Validation protocol **[v1.1]**

Any implementation of the equation must be validated before any results are reported. v1.0 made three of its five criteria depend on the ablation modes of §6 (unitary limit, harmonic eigenstate, thermal limit); those criteria cannot be run under §1.6 and are withdrawn. The protocol is restated below with what the complete equation, noise always on, can establish.

### 7.1 Refusals

The implementation must refuse, with an error at parameter construction and at the entry of every integrator, each of: a `mode` other than the complete one; $\Gamma \leq 0$; $f_{\text{FDT}} \leq 0$; $k_B T \leq 0$; $M < 3$; `len(nu) != len(lam)`. A refusal that can be bypassed through the public interface is a failure of this item.

### 7.2 Norm conservation (unitary limit) — withdrawn **[v1.1]**

Required `mode = "linear"`. Not runnable under §1.6.

### 7.3 Harmonic oscillator eigenstates / thermal limit — withdrawn **[v1.1]**

Required `mode = "linear"` and `mode = "thermal"` respectively. Not runnable under §1.6.

### 7.4 Reference solver comparison

If a reference implementation is available, run the two side-by-side with identical parameters, identical seed, identical initial state **and identical RNG convention** (Appendix B: one `default_rng(seed)`, two `standard_normal(N)` calls per step, real part first). With the RNG stream identical, the comparison is deterministic even though noise is on.

| Quantity | Tolerance |
|---|---|
| relative L2 error in $|\Psi|^2$ at $t = T$, same seed | $\leq 10^{-3}$ |
| distribution of late-time observables (§8.1) across seeds, KS test | $p > 0.05$ |

A discrepancy above tolerance is a bug; track it down before proceeding.

### 7.5 Convergence check

With noise on, a run at $dt/2$ draws a different noise realisation from a run at $dt$, so the deterministic $\mathcal{O}(dt^2)$ comparison of v1.0 is not available. Convergence is checked weakly: for one canonical configuration, run an ensemble of seeds at $dt$ and at $dt/2$ (same seed set), and compare the distributions of late-time observables (§8.1) by KS test, $p > 0.05$. Likewise for $dx$ versus $dx/2$ at fixed $L$.

### 7.6 Acceptance criteria

A solver is considered validated when:

1. Every refusal of §7.1 is in force and covered by a test.
2. Match against the reference solver within the tolerances of §7.4.
3. Weak convergence in $dt$ and in $dx$ per §7.5.

Until these hold, do not report results.
...
**Norm**: $\|\Psi\|^2 = \int |\Psi|^2 d^D x$. Not conserved: it reaches the finite FDT plateau (§12.1). **[v1.1]**
...
### 10.2 Dispersive (kinetic dominates) — withdrawn **[v1.1]**

v1.0 defined this regime as a "no nonlinearity" baseline used as a control. There are no controls (§1.6). The v1.0 text is in the archived copy.

### 10.3 Anti-collapse (3D supercritical NLS with memory)

$\Lambda < 0$ with $|\Lambda| \gg 1$ (strongly attractive, blow-up regime), $\lambda_j > 0$ (repulsive memory), no $V_{\text{ext}}$, with FDT **[v1.1]** (the bath is always on):
...
The 3D cubic NLS is $L^2$-supercritical; a localised initial state with $\Lambda < 0$ would blow up in finite time (peak density diverges, support shrinks to a point). With the memory mechanism active, peak density rises briefly, then falls by 4–5 orders of magnitude, effective spatial support rises by 4–5 orders of magnitude, and the field stabilises in a bounded distributed configuration. This is the structural anti-collapse demonstration.

**[v1.1]** v1.0 prescribed a matched control with $\lambda_j = 0$. That is an ablation and is withdrawn (§1.6).

### 10.4 Crystallisation (R5 series)

$\Lambda = -8$, hierarchical memory with $\Sigma_\lambda$-split structure, no $V_{\text{ext}}$, with FDT **[v1.1]** (v1.0 also listed a unitary variant with $\Gamma = 0$, $f_{\text{FDT}} = 0$; withdrawn, §1.6):
...
### 10.5 Failure modes — withdrawn **[v1.1]**

v1.0 listed five configurations "used as controls", each a single-term ablation (no memory, no noise, no dissipation, no cubic and no memory, no $V_{\text{ext}}$ and no memory), and concluded that the full equation occupies a point none of its ablations reaches. Under §1.6 none of these configurations is run: each is refused. What the full equation does is read from the full equation. The v1.0 text is in the archived copy.
...
- **Structure-forming (the equation)**: rise to the FDT plateau, possibly with transient over/undershoot during initial settling. The value of the plateau is read from the run.

**[v1.1]** v1.0 also described a linear/unitary line at 1.0 and a thermal line; both required modes that no longer exist. Norm not reaching a plateau means insufficient run time or a bug in the FDT scaling.

### 12.2 Peak density vs. spatial support

Plot $\rho_{\max}(t)$ and $P(t)$ (participation ratio) on log scale.

- ~~Collapse trajectory (no-memory)~~ and ~~dispersive trajectory (no nonlinearity)~~ **[v1.1]** withdrawn: both are ablations (§1.6). The shapes below are the equation's own.
...
**[v1.1]** v1.0 had a sixth item, an ablation comparison, and called it mandatory. It is withdrawn (§1.6).
...
sigma = 1.5
Gamma = 0.05
f_FDT = 0.002     # declared positive: η is on; magnitude comes from (3c)
kT    = 1.0       # temperature dial under the lock (3c)
nu    = (2.0, 0.5, 0.1)
lam   = (-0.3, -0.2, -0.1)
L     = 32.0
N     = 128
...
**[v1.1]** `mode = "full"` is removed: there is one mode. Every coupling is non-zero and the memory field has three time-scales; this is the only default set in this document that satisfies §1.6 as written.

### A.2 3D structure-forming (R5 default) — withdrawn **[v1.1]**

The v1.0 set had $\Gamma = 0$, $T_{\text{bath}} = 0$, $\alpha = 0$ and two memory time-scales. Each of those switches a term off (§1.6); the reference implementation refuses the set. It is preserved in the archived v1.0 copy and is not a valid input. A 3D default satisfying §1.6 is not stated in this document until the author states it.

### A.3 3D anti-collapse (focusing supercritical NLS with memory) — withdrawn **[v1.1]**

The v1.0 set had $\Gamma = 0$, $f_{\text{FDT}} = 0$, $\alpha = 0$, two memory time-scales, and instructed a comparison against $\lambda_j = (0, 0)$. Both the set and the comparison are ablations under §1.6; the reference implementation refuses the set. Preserved in the archived v1.0 copy; not a valid input.
...
## Appendix C — Validation reference numbers — withdrawn **[v1.1]**

The v1.0 table gave expected values per mode (`linear` / `full`) for A.1 and a no-memory / with-memory pair for A.3. The `linear` column and the no-memory column are ablations (§1.6). The `full` column was produced by the v1.0 skeleton with the declared $f_{\text{FDT}} = 0.002$ applied literally in (4), not with the lock (3c); its numbers are therefore not reference values for the equation as specified in v1.1. No replacement numbers are stated here: reference numbers are produced by running the reference implementation at A.1 and recording the output with seed, code revision and date, and are entered in this appendix only by the author. The v1.0 table is preserved in the archived copy.

---

## Appendix D — Reference code skeleton **[v1.1]**

A minimal self-contained reference: the 1D solver of the complete equation, with the refusals of §1.6, the lock (3c), and $H_{\text{lin}}$ as in equation (8). The v1.0 skeleton carried a `mode` switch, wrote the kinetic term as `hbar * k**2` (inconsistent with equation (8)) and applied the declared $f_{\text{FDT}}$ literally; it is preserved in the archived copy. 3D extension follows §5.

```python
"""
Triad equation 1D reference solver — complete equation only (§1.6).
...
class TriadParams:
    ...
    sigma: float = 1.5
    Gamma: float = 0.05
    f_FDT: float = 0.002        # declared positive; magnitude from (3c)
    kT:    float = 1.0
    ...
    def __post_init__(self):
        # §1.6 item 2: refuse anything that switches a term off.
        # the same refusals as the reference implementation, and no others
        if len(self.nu) != len(self.lam): raise ValueError("triad rule: len(nu) must equal len(lam)")
        if len(self.nu) < 3:  raise ValueError("triad rule: P2 memory field must have at least 3 time-scales")
        if self.Gamma <= 0:   raise ValueError("triad rule: P3 bath requires Gamma > 0")
        if self.f_FDT <= 0:   raise ValueError("triad rule: P3 bath requires f_FDT > 0 to keep η active")
        if self.kT <= 0:      raise ValueError("triad rule: P3 bath requires kT > 0 to keep η active")
...
def integrate(p: TriadParams):
    ...
    # initial state (§9.1), normalised
    psi = np.exp(-x**2 / 8.0).astype(np.complex128)
    psi /= np.sqrt(np.sum(np.abs(psi)**2) * dx)

    # memory fields (eq. 5)
    M = len(p.nu)
    nu = np.asarray(p.nu, dtype=np.float64)
    lam = np.asarray(p.lam, dtype=np.float64)
    y = np.zeros((M, p.N), dtype=np.float64)

    # linear half-step propagator (eq. 8, eq. 9)
    H_lin_k = p.hbar**2 * k**2 / (2*p.m) + p.alpha * abs_k**p.sigma
    half_lin = np.exp(-1j * H_lin_k * p.dt / (2*p.hbar) - p.Gamma * p.dt / (2*p.hbar))

    # FDT lock (eq. 3c) and per-site noise amplitude (eq. 4)
    f_FDT_eff = 2.0 * p.Gamma * p.kT * dx / p.hbar        # D = 1
    noise_amp = np.sqrt(f_FDT_eff * p.dt / dx)

    # memory update: Euler if max(nu) dt < 0.05, else exact OU (§3.4)
    use_ou = float(np.max(nu) * p.dt) >= 0.05
    ou = np.exp(-nu * p.dt)
    ...
    for step in range(n_steps + 1):
        ...
        # §3.6 — Strang split-step
        psi = np.fft.ifft(np.fft.fft(psi) * half_lin)                 # 1
        rho = np.abs(psi)**2                                           # 2
        V_tot = V_ext + p.Lambda * rho + (lam[:, None] * y).sum(axis=0)  # 3
        psi = psi * np.exp(-1j * V_tot * p.dt / p.hbar)                # 4
        if use_ou:                                                     # 5 (13b)
            y = ou[:, None] * y + (1.0 - ou)[:, None] * rho
        else:                                                          # 5 (13a)
            y = y + p.dt * nu[:, None] * (rho - y)
        xi = (rng.standard_normal(p.N) + 1j * rng.standard_normal(p.N)) / np.sqrt(2.0)
        psi = psi + noise_amp * xi                                     # 6
        psi = np.fft.ifft(np.fft.fft(psi) * half_lin)                 # 7

    return {"t": rec_t, "x": x, "dx": dx, "density": rec_density, "psi_final": psi,
            "y_final": y, "params": p}


if __name__ == "__main__":
    out = integrate(TriadParams())
    norm_t = out["density"].sum(axis=0) * out["dx"]
    print(f"norm in [{norm_t.min():.4f}, {norm_t.max():.4f}]  (FDT plateau; never exactly conserved)")
```

Running this script prints one line: the norm range over the run. Under FDT balance the norm is not conserved and settles on a plateau; there is no unitary line to compare against, because there is no unitary mode.
