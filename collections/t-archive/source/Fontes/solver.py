...
def _default_D() -> int:
    d = _env('TRIADLANG_D', 3)
    if d == 0:
        d = 3
    if d == 2:
        raise ValueError('TRIADLANG_D=2 is not a path; use 3 (default) or 1 (diagnostic)')
    if d < 1:
        raise ValueError(f'TRIADLANG_D={d} invalid (use D=3 default, D=1 diagnostic, or 0=auto→3)')
    return d
...
class TriadParams:
    ...
    def __post_init__(self):
        ...
        if self.native_backend not in ('auto', 'cpu', 'metal'):
            raise ValueError("native_backend must be auto, cpu, or metal")
        nu = tuple(self.nu)
        lam = tuple(self.lam)
        if len(nu) != len(lam):
        ...
        if any((not np.isfinite(v)) or v <= 0 for v in nu):
            raise ValueError(f'triad rule: every P2 decay rate must be finite and > 0, got {nu}')
        if any(not np.isfinite(v) for v in lam) or not any(abs(v) > 0 for v in lam):
            raise ValueError(f'triad rule: P2 must influence the present field through non-zero finite lam, got {lam}')
        if self.mode != 'triad':
        ...
        if not getattr(self, 'fdt_couple', False) and self.f_FDT <= 0:
            raise ValueError(f'triad rule: P3 bath requires f_FDT > 0 when fdt_couple=False, got {self.f_FDT}')
        if getattr(self, 'fdt_couple', False) and getattr(self, 'kT', 1.0) <= 0:
            raise ValueError(f'triad rule: P3 bath requires kT > 0 when fdt_couple=True, got {getattr(self, "kT", 1.0)}')
...
def _validate_triad_params(p: TriadParams) -> None:
    ...
    if not getattr(p, 'fdt_couple', False) and p.f_FDT <= 0:
        raise ValueError(f'triad rule: P3 bath requires f_FDT > 0 when fdt_couple=False, got {p.f_FDT}')
    if getattr(p, 'fdt_couple', False) and getattr(p, 'kT', 1.0) <= 0:
        raise ValueError(f'triad rule: P3 bath requires kT > 0 when fdt_couple=True, got {getattr(p, "kT", 1.0)}')
...
def _native_lib():
    with _NATIVE_LIB_LOCK:
        ...
        if _NATIVE_LIB is None:
            ...
            native_root = os.path.join(repo_root, 'native', 'c')
            candidates = [
                os.path.join(native_root, 'libtriad_rt.dylib'),
                os.path.join(native_root, 'libtriad_rt.so'),
            ]
            candidates.extend(sorted(glob.glob(os.path.join(native_root, 'build', '*', 'libtriad_rt.dylib'))))
            candidates.extend(sorted(glob.glob(os.path.join(native_root, 'build', '*', 'libtriad_rt.so'))))
            so_path = next((path for path in candidates if os.path.exists(path)), None)
            if so_path is None:
                raise RuntimeError(
                    f'native runtime missing in {native_root}; compile with: make -C native/c')
            try:
...
def _try_native_c(p, **kwargs):
    ...
    try:
        ...
        if getattr(p, 'native_backend', 'auto') == 'metal':
            raise RuntimeError(f'Metal native backend requested but unavailable: {_e}') from _e
        return None, False
    ...
    native_backend = getattr(p, 'native_backend', 'auto')
    sp.backend = {'auto': 0, 'cpu': 1, 'metal': 2}[native_backend]

    def _backend_name(code: int) -> str:
        return {1: 'native_cpu', 2: 'native_metal'}.get(int(code), 'native_unknown')

    def _checked_backend(result, free_fn) -> int:
        code = int(result.backend_used)
        if code == 0 or not bool(result.psi_final):
            free_fn(ctypes.byref(result))
            raise RuntimeError(
                'native Metal was requested but the Metal device or kernel was unavailable')
        return code
    ...
    try:
        ...
        if D == 1:
            ...
            backend_code = _checked_backend(r, _lib.triad_solver_result_free)
            psi = _cplx_to_numpy(r.psi_final, n, (n,))
            x = _double_to_numpy(r.x, (n,))
            density = _double_to_numpy(r.density_final, (n,))
            ...
            return dict(
                ...
                density=density.reshape(n, 1),
                psi_final=psi,
                y_final=y_final,
                params=p,
                backend_used=_backend_name(backend_code),
            ), True
            ...
            backend_code = _checked_backend(r, _lib.triad_solver_result_2d_free)
            ...
            backend_code = _checked_backend(r, _lib.triad_solver_result_3d_free)
            ...
            peak_t = _double_to_numpy(r.peak_t, (nrec,)) if nrec else np.zeros(0)
            participation_t = _double_to_numpy(r.participation_t, (nrec,)) if nrec else np.zeros(0)
            rec_every_native = 4
            t_rec = np.arange(nrec, dtype=np.float64) * rec_every_native * p.dt
            dx = r.dx
            ...
            backend_code = _checked_backend(r, _lib.triad_solver_result_nd_free)
            shape = (n,) * D
            total = n ** D
            ...
            rec_every_native = 4
            return dict(
                t=np.arange(nrec, dtype=np.float64) * rec_every_native * p.dt,
                x=x, dx=dx,
                psi_final=psi,
                y_final=y_final,
...
def integrate(p: TriadParams, psi0: Optional[np.ndarray]=None, y0: Optional[np.ndarray]=None, auto_halve_dt: bool=True, noise_provider: Optional[Callable]=None, record_y: bool=False) -> dict:
    ...
    for step in range(n_steps + 1):
        ...
        if bc_mask is not None:
            psi = psi * bc_mask
    result = {'t': np.asarray(rec_t), 'x': asnumpy(x), 'dx': dx, 'density': np.asarray(rec_density).T if rec_density else np.zeros((p.N, 0)), 'psi_final': asnumpy(psi), 'y_final': asnumpy(y), 'params': p, 'backend_used': 'numpy' if xp is np else 'cuda'}
    if record_y:
        if rec_y:
            ya = np.asarray(rec_y)
...
def integrate_2d(p: TriadParams, psi0: Optional[np.ndarray]=None, y0: Optional[np.ndarray]=None, auto_halve_dt: bool=True, record_y: bool=False, record_density: bool=False) -> dict:
    ...
    for step in range(n_steps + 1):
        ...
        if bc_mask is not None:
            psi = psi * bc_mask
    result = {'t': np.asarray(rec_t), 'x': asnumpy(xs), 'dx': dx, 'psi_final': rec_psi_final, 'y_final': asnumpy(y), 'params': p, 'backend_used': 'numpy' if xp is np else 'cuda'}
...
def prepare_memory_nls_3d(p: TriadParams, xp) -> dict:
    """Prepare the exact 3-D Strang/Memory-NLS law used by ``integrate_3d``.

    This is intentionally a numerical primitive rather than a second solver.
    Persistent experiments can add a time-dependent external potential while
    retaining the core field, memory, damping, dispersion and FDT equations.
    """
    _validate_triad_params(p)
    if p.D != 3:
        raise ValueError(f"prepare_memory_nls_3d requires D=3, got D={p.D}")
    dx = p.L / p.N
    kvec = 2.0 * xp.pi * xp.fft.fftfreq(p.N, d=dx)
    KX, KY, KZ = xp.meshgrid(kvec, kvec, kvec, indexing='ij')
    k2 = KX * KX + KY * KY + KZ * KZ
    eff = _effective_params(p, dx=dx, D=3)
    H_lin_k = (p.hbar ** 2 * k2 / (2.0 * p.m)
               + eff['alpha'] * xp.sqrt(k2) ** p.sigma)
    half_linear = xp.exp(
        -1j * H_lin_k * p.dt / (2.0 * p.hbar)
        - eff['Gamma'] * p.dt / (2.0 * p.hbar))
    module_name = getattr(xp, '__name__', '')
    real_dtype = xp.float32 if module_name.startswith('mlx') else xp.float64
    memory_decay_half = xp.exp(
        -xp.asarray(p.nu, dtype=real_dtype) * p.dt * 0.5)
    noise_amplitude = float((eff['f_FDT_e'] * p.dt / dx ** 3) ** 0.5)
    return {
        'dx': dx,
        'k2': k2,
        'half_linear': half_linear,
        'memory_decay_half': memory_decay_half,
        'memory_couplings': xp.asarray(eff['lam'], dtype=real_dtype),
        'nonlinearity': eff['Lambda'],
        'noise_amplitude': noise_amplitude,
    }


def memory_nls_strang_step_3d(
        psi, y, *, xp, law: dict, external_potential, dt: float,
        hbar: float, normal_provider: Callable, bc_mask=None,
        return_terms: bool=False):
    """Advance one exact core 3-D Strang/Memory-NLS step.

    ``external_potential`` may vary in time, but observer values are never
    accepted here.  The bath samples are supplied by the owning universe so a
    checkpoint can preserve its random trajectory.
    """
    half_linear = law['half_linear']
    psi = xp.fft.ifftn(xp.fft.fftn(psi) * half_linear)
    rho = xp.abs(psi) ** 2

    memory_count = int(y.shape[0])
    if memory_count:
        shape = (memory_count, 1, 1, 1)
        decay = law['memory_decay_half'].reshape(shape)
        y = decay * y + (1.0 - decay) * rho
        v_mem = (law['memory_couplings'].reshape(shape) * y).sum(axis=0)
    else:
        v_mem = xp.zeros_like(rho)

    potential = external_potential + law['nonlinearity'] * rho + v_mem
    psi = psi * xp.exp(-1j * potential * dt / hbar)

    rho_local = xp.abs(psi) ** 2
    if memory_count:
        y = decay * y + (1.0 - decay) * rho_local

    noise_amplitude = law['noise_amplitude']
    if noise_amplitude > 0.0:
        xi = normal_provider(tuple(psi.shape))
        xip = normal_provider(tuple(psi.shape))
        psi = psi + noise_amplitude * (xi + 1j * xip) / xp.sqrt(2.0)

    psi = xp.fft.ifftn(xp.fft.fftn(psi) * half_linear)
    if bc_mask is not None:
        psi = psi * bc_mask
    terms = {'rho_local': rho_local, 'v_mem': v_mem} if return_terms else None
    return psi, y, terms
...
def integrate_3d(p: TriadParams, psi0: Optional[np.ndarray]=None, y0: Optional[np.ndarray]=None, auto_halve_dt: bool=True, record_density: bool=False, record_y: bool=False) -> dict:
    ...
    if y0 is None:
        ...
        y = xp.asarray(y0, dtype=xp.float64).copy()
    core_law = prepare_memory_nls_3d(p, xp)
    half_lin = core_law['half_linear']
    ou_decay_half = core_law['memory_decay_half']
    noise_amp = core_law['noise_amplitude']
    bc = getattr(p, 'bc', 'periodic')
    if bc == 'absorbing':
        mask_1d_np = _build_absorbing_mask(p.N, p.L, getattr(p, 'bc_width', 0.15))
    ...
    for step in range(n_steps + 1):
        ...
        if not exptrap_3d:
            psi, y, _ = memory_nls_strang_step_3d(
                psi, y, xp=xp, law=core_law, external_potential=V_ext,
                dt=p.dt, hbar=p.hbar,
                normal_provider=rng.standard_normal, bc_mask=bc_mask)
            continue
        ...
        if bc_mask is not None:
            psi = psi * bc_mask
    result = {'t': np.asarray(t_arr), 'x': asnumpy(xs), 'dx': dx, 'psi_final': asnumpy(psi), 'y_final': asnumpy(y), 'peak_t': np.asarray(peak_density_t), 'participation_t': np.asarray(participation_t), 'params': p, 'backend_used': 'numpy' if xp is np else 'cuda'}
    if record_density and rec_density:
...
def _integrate_highd(p: TriadParams, psi0: Optional[np.ndarray]=None,
    ...
    ou_decay_half = xp.exp(-nu_arr * p.dt * 0.5) if M else None
    noise_amp = float(xp.sqrt(xp.asarray(f_FDT_e * p.dt / vol))) if f_FDT_e > 0 else 0.0
    ...
    result = {
        ...
        'params': p,
        'backend_used': 'numpy' if xp is np else 'cuda',
    }
    if record_density and rec_density:
...
if __name__ == '__main__':
    out = integrate(TriadParams(T=0.2, seed=0, N=16))
