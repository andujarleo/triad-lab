#!/usr/bin/env python3
# Run 33 — ninho +/- (I0 / environment). Ima / anti-colapso.
# Triade completa, Theta_core congelado (Q00). Standalone 3D Strang.
# 1 gaussiana = 1 atomo. Inner atom INSIDE the outer atom, same center.
# Outer phase 0 (+), inner phase pi (-): Psi = G_out - G_in, then normalize.
# NOT side-by-side (run 32). NOT same-sign nest (run 31).
...
OUT_VAULT = VAULT / "Artefatos" / "triad_ninho_pm"
FALLBACK = Path("/tmp/triad_ninho_pm")
RUN_ID = "33"
RUN_NAME = "triad_ninho_pm"
...
def main():
    ...
    print("=== 33 ninho_pm I0 ===", flush=True)
    ...
    iso_used = {}
    fig_specs = [
        ("ic", "01_ic_isosurface.png", "33 IC  rho  ninho +/-  s=0.5 subset s=2.0  t={t:.2f}"),
        ("early", "01b_isosurface_early.png", "33 isosuperficie rho early  t={t:.2f}  (banho kT=1)"),
        ("mid", "02_isosurface_mid.png", "33 isosuperficie rho mid  t={t:.2f}"),
        ("late", "03_isosurface_late.png", "33 isosuperficie rho late  t={t:.2f}  universo no cubo"),
    ]
    ...
    if "t002" in volumes_rho:
        ...
        used = save_rho_iso_fig(
            ...
            dx,
            LBOX,
            "33 isosuperficie rho t=%.3f  (se o ninho ja morreu, e isto)" % tt,
        )
    ...
    if "ic" in volumes_re:
        used_re = save_re_iso_fig(
            ...
            dx,
            LBOX,
            "33 IC  Re(Psi)  vermelho + / azul -  t=0",
        )
    ...
    if "early" in volumes_re:
        used_re = save_re_iso_fig(
            ...
            dx,
            LBOX,
            "33 Re(Psi) early t=%.2f  +/-" % snap_t["early"],
        )
    ...
    if "late" in volumes_re:
        used_re = save_re_iso_fig(
            ...
            dx,
            LBOX,
            "33 Re(Psi) late t=%.2f  +/-" % snap_t["late"],
        )
    ...
    for ax, (key, lab) in zip(axes.ravel(), order):
        ...
        ax.set_xlabel("x")
        ax.set_ylabel("y")
    fig.suptitle("33 — midplane xy  |Psi|^2  (ninho +/-, Theta_core, kT=1)")
    fig.tight_layout()
    ...
    for ax, (key, lab) in zip(axes.ravel(), order):
        ...
        ax.set_xlabel("x")
        ax.set_ylabel("y")
    fig.suptitle("33 — midplane xy  Re(Psi)  vermelho + / azul -  (halo +, buraco no centro)")
    fig.tight_layout()
    ...
    for key, fname in (
        ...
        save_midplane(
            out / fname,
            sl,
            "33 midplane xy  |Psi|^2  %s  t=%.3f" % (key, snap_t.get(key, 0)),
            cmap="magma",
            cbar_label="|Psi|^2",
        )
    ...
    for key, fname in (
        ...
        save_midplane(
            out / fname,
            sl,
            "33 midplane xy  Re(Psi)  %s  t=%.3f" % (key, snap_t.get(key, 0)),
            cmap="RdBu_r",
            vmin=-vmax,
            vmax=vmax,
            cbar_label="Re(Psi)",
    ...
    ax.set_title("33 — corte Re(Psi) ao longo de x  (buraco/nucleo no origem, envelope +)")
    ...
    ax.set_title("33 — Re(Psi) t=0 zoom  (halo + / buraco no centro)")
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=8)
    fig.tight_layout()
    ...
    ax.set_title("33 — perfil radial  |Psi|^2  (oco no origem, casca s~0.5-2)")
    ax.set_xlim(0, 16)
    ...
    ax.set_xlabel("r")
    ax.set_ylabel("rho(r)")
    ax.set_title("33 — perfil radial linear  (oco t=0, early, t=0.02)")
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=8)
    fig.tight_layout()
    ...
    axes[0, 0].set_title("peak rho  (fica finito?)")
    ...
    fig.suptitle("33 ninho +/- — peak / PR / R_rms / norma  (Theta_core, kT=1, sem retune)")
    fig.tight_layout()
    ...
    for a in axes.ravel():
        a.set_xlabel("t")
        a.grid(True, alpha=0.3)
    fig.suptitle("33 — duas escalas e ninho de sinal vs t")
    fig.tight_layout()
    ...
    fig.suptitle("33 — zoom t<=0.4  (o ninho +/- some aqui se kT=1 preencher)")
    fig.tight_layout()
    ...
    ax.grid(True, alpha=0.3)
    fig.suptitle("33 ninho +/- — I0, Theta_core, kT=1, N=64  (sinal / duas escalas / pico finito)")
    fig.tight_layout()
