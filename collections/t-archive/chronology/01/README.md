**English** · [Português](README.pt-BR.md) · [Collection](../../README.md)

# 01 · Early field relations

Exploratory 2D relations before the reference specification.

Imported historical record; this organization did not rerun the simulation.

[Read the original note](../../reading/Simula%C3%A7%C3%B5es/01%20triad_sim.md) · [Every file](../../files/run-01.md)

No dedicated runner is linked in this entry. Consult the note and complete record for historical listings and dependencies.

![Original run figure](../../source/Artefatos/triad_sim/00_montagem_simulacao.png)

## Available material

7 files associated with this entry: 1 `.md`, 6 `.png`.

[Timeline](../../guides/timeline.md) · [02 →](../02/README.md)
